...
def add_vasp_arguments(parser, calc_type):
    ...
    parser.add_argument(
        "-c", "--config", dest="config", required=False, help="path to the config file"
    )
    ...
    parser.add_argument(
        "--spin", dest="spin", action="store_true", default=False, help=msg
    )
    ...
    if calc_type == "opt" or calc_type == "eos":
        ...
        parser.add_argument(
            "--train", dest="is_training", action="store_true", help=msg
        )
    return parser


def get_args(args) -> dict:
    """Process and validate command line arguments for VASP calculations.
    ...
    if args["setups"] is not None:
        all_chemical_symbols = list(
            set([atom.symbol for structure in structures for atom in structure])
        )
