...
def run_mace_opt(args: dict, calc_type: str):
    ...
    try:
        ...
        try:
            ...
            if not converged:
                ...
                print(f"Consider increasing --nsteps or relaxing --fmax")
        ...
        if not isinstance(final_energy, (int, float)) or not np.isfinite(final_energy):
            raise RuntimeError(f"Optimization produced invalid final energy: {final_energy}")
        
        if not isinstance(final_forces, np.ndarray) or not np.all(np.isfinite(final_forces)):
    ...
    try:
        ...
        raise RuntimeError(f'Failed to write final trajectory file to {outdir / "final.traj"}: {e}')
    except Exception as e:
