...
def run_mace_eos(args: dict, calc_type: str):
    ...
    try:
        ...
        if np.linalg.det(cell) <= 0:
            raise RuntimeError(
                "Reference structure has invalid cell (zero or negative volume)"
            )
    ...
    try:
        for i, x in enumerate(scales):
            try:
                ...
                try:
                    calc_struct: Atoms = read(
                        new_args["outdir"] / "final.traj", index=-1
                    )
    ...
    if len(volumes) < 5:
        cleanup_eos_temp_files(outdir, completed_scales)
        raise RuntimeError(
            f"Insufficient data points for EOS fitting: {len(volumes)} < 5"
        )
    ...
    try:
        ...
        if not all(np.isfinite([v0, e0, B])):
            raise RuntimeError(
                f"EOS fit produced invalid results: v0={v0}, e0={e0}, B={B}"
            )
    ...
    try:
        ...
        cleanup_eos_temp_files(outdir, completed_scales)
        raise RuntimeError(
            f"Permission denied writing EOS data files to {outdir}. Check directory permissions."
        )
    except OSError as e:
        cleanup_eos_temp_files(outdir, completed_scales)
