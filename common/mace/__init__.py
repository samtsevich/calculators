...
def add_mace_arguments(parser, calc_type: str):
    ...
    parser.add_argument("--default_dtype", dest="default_dtype", default=DEFAULT_DTYPE, help=msg)

    msg = 'PyTorch compilation mode for performance optimization'
    parser.add_argument("--compile_mode", dest="compile_mode", default=DEFAULT_COMPILE_MODE, help=msg)
...
def get_args(args: Dict, calc_type: str) -> Dict:
    ...
    if model_path.suffix not in ['.model', '.pth']:
        raise ValueError(f'MACE model file must have .model or .pth extension, got {model_path.suffix}')
    ...
    assert device in valid_devices, f'Device must be one of {valid_devices}, got {device}'
    ...
    assert dtype in valid_dtypes, f'Data type must be one of {valid_dtypes}, got {dtype}'
    ...
    if calc_type in ['opt', 'eos']:
        ...
        if not isinstance(fmax, (int, float)):
            raise ValueError(f'Force convergence criterion (fmax) must be a number, got {type(fmax).__name__}')
        if fmax <= 0:
            raise ValueError(f'Force convergence criterion (fmax) must be positive, got {fmax}')
        
        # Validate nsteps
        if not isinstance(nsteps, int):
            raise ValueError(f'Number of steps (nsteps) must be an integer, got {type(nsteps).__name__}')
        if nsteps <= 0:
        ...
        if model_path.suffix == '.model':
            ...
            if not any(key in state_dict for key in expected_keys):
                ...
                if not isinstance(state_dict, dict):
                    raise ValueError("Model file does not contain a valid state dictionary")
